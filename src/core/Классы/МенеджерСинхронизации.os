...
Перем ПроизвестиОчисткуКаталога;
...
Процедура РазложитьМодули1СПоПапкамСогласноИерархииМетаданных(Знач КаталогПлоскойВыгрузки, Знач КаталогИерархическойВыгрузки, Знач Формат) Экспорт
	...
	Если ПроизвестиОчисткуКаталога Тогда
		Лог.Отладка("Производим очистку каталога выгрузки");
		ОчиститьЦелевойКаталогВыгрузки(КаталогИерархическойВыгрузки);
	Иначе
		Лог.Отладка("Не очищаем каталог выгрузи");
	КонецЕсли;
...
Функция ТолькоДобавлены(ТаблицаИзменений, Знач НомерВерсии)

	НомерВерсии = СокрЛП(НомерВерсии);

	Отбор = Новый Структура();
	Отбор.Вставить("VERNUM", НомерВерсии);
	
	ТабСтроки = ТаблицаИзменений.НайтиСтроки(Отбор);
	
	Для каждого Стр Из ТабСтроки Цикл
		Если (Число(Стр.SELFVERNUM) > 1) и (Стр.OBJNAME <> "Конфигурация") Тогда
			Возврат Ложь;
		КонецЕсли;
	КонецЦикла;
...
Процедура СинхронизироватьХранилищеКонфигурацийСГит(Знач КаталогРабочейКопии, 
	...
	Если УмныйРежим = 1 Тогда
		Лог.Отладка("Умный режим. Очистка каталога выгрузки когда есть не только добавление");
		ТаблицаИзменений = ПолучитьТаблицуИзменеий(ФайлХранилища);
	КонецЕсли;
	...
	Пока СледующаяВерсия <= МаксимальнаяВерсияДляРазбора Цикл
		...
		Если СтрокаВерсии <> Неопределено Тогда
			...
			ПроизвестиОчисткуКаталога = Истина;

			Лог.Отладка("Фиксируем в файле VERSION версию " + СледующаяВерсия);
			ЗаписатьФайлВерсийГит(КаталогРабочейКопии, СледующаяВерсия);

			Если УмныйРежим = 1 Тогда
				Лог.Отладка("Определяем возможность не удалять файлы");
				ПроизвестиОчисткуКаталога = НЕ ТолькоДобавлены(ТаблицаИзменений, СледующаяВерсия);
			КонецЕсли;
...
Функция ПолучитьТаблицуИзменеий(Знач ФайлХранилища)

	ЧтениеБазыДанных = Новый ЧтениеТаблицФайловойБазыДанных;
	ЧтениеБазыДанных.ОткрытьФайл(ФайлХранилища);
	Попытка
		ТаблицаБД = ЧтениеБазыДанных.ПрочитатьТаблицу("HISTORY");
	Исключение
		ЧтениеБазыДанных.ЗакрытьФайл();
		ВызватьИсключение;
	КонецПопытки;
...
КонецФункции // ПолучитьТаблицуИзменеий(Знач ФайлХранилища)
